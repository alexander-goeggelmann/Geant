...
#include "PhysicsList/include/Cresst_PhysicsList.hh"
...
int main(int argc,char** argv)
{
    ...
    G4VModularPhysicsList *physicsList = new Cresst_PhysicsList();
    ...
    if (input.is_open())
    {
        ...
        while (std::getline(input, line))
        {
            ...
            else if (boost::algorithm::contains(line, "thickness")) d = get_integer(parameter);
            else if (line.size() > 0)
            {
    ...
    Cresst_DetectorConstruction *detector = new Cresst_DetectorConstruction(d);
    runManager->SetUserInitialization(detector);


    Cresst_ActionInitialization *act = new Cresst_ActionInitialization(
        detector, x_seed, y_seed, nuclide, d);
